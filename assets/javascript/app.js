...
function signInValidation() {
  ...
  database.ref("/userAccounts/").on("child_added", function(snapshot) {
    if (
      ...
      setTimeout(function() {
        database.ref('/userData/' + username).once('value', function(snapshot) {
          if(snapshot.val().hasOwnProperty('favoriteCityArr')) {
            favoriteCityArr = snapshot.val().favoriteCityArr;
            
            favoriteCityArr.forEach(function(x) {
              var newBtn = $('<button>').text(x).addClass('favorite-cities svd-btn btn btn-outline-danger').attr('id', x);
...
function addToFavorite(city) {
  favoriteCityArr.push(city);
  database.ref('/userData/'+username).set({
    favoriteCityArr
  });
  var newBtn = $('<button>').text(city).addClass('favorite-cities svd-btn btn btn-outline-danger').attr('id', city);
  $('#saved-Cities').append(newBtn);
};
  ...
  $.ajax(setting)
    ...
    .then(function(response) {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        var newLi = $('<li>').html(`<a href=${eventUrl} target="_blank">${eventTitle}</a>`);
        if(i % 2 === 0) {
          newLi.addClass("list-group-item list-group-item-dark");
        } else {
          newLi.addClass("list-group-item list-group-item-danger");
        }
        $('#upcoming-events').append(newLi);
      }
    });
}
...
$("#search-btn").on("click", function() {
  ...
  let searchInput = $("#input-city").val().trim().replace(/(^|\s)\w/g, (x)=> x.toUpperCase());
  console.log(searchInput)
  $('#dash-city').text(searchInput)
...
$(document).on('click', '.favorite-cities', function() {
  console.log(this.id);
